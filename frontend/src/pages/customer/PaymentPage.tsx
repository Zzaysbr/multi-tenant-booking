...
import { QrCode, Upload, ArrowLeft, Loader2, Camera, Calendar, Clock, Info } from 'lucide-react';
      ...
      <div className="max-w-2xl mx-auto px-6 mt-12 space-y-10">
        ...
        <section className="card-cozy p-12! text-center space-y-10">
           ...
           <div className="bg-secondary p-8 rounded-[48px] inline-block border-4 border-white shadow-inner">
             {/* ✅ ใช้ getFullImageUrl ดึง QR ร้าน */}
             {shop?.qrCodeUrl ? <img src={getFullImageUrl(shop.qrCodeUrl)} className="w-64 h-64 object-cover rounded-[36px] shadow-sm" /> : <div className="w-64 h-64 flex items-center justify-center text-stone-200">No QR Code</div>}
           </div>
        ...
        <form onSubmit={hSubmit} className="space-y-8 pt-4">
           ...
           {/* slipPreview ใช้ URL.createObjectURL ซึ่งเป็น blob: ไม่ต้องครอบ getFullImageUrl */}
