...
export default function BookingPage() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setServices(i.data.services || []); setStaffs(i.data.staffs || []); 
        setBusinessHours(i.data.businessHours || []); setShopName(c.data.config.name);
  ...
  useEffect(() => {
    if (selStaff && selDate) {
      api.get(`/${tenantPath}/bookings/busy-slots`, { params: { staffId: selStaff.id, date: selDate } }).then(res => setBusySlots(res.data.busy || []));
    }
      ...
      <div className="max-w-5xl mx-auto px-6 mt-12 grid grid-cols-1 lg:grid-cols-12 gap-12">
        <div className="lg:col-span-7 space-y-12">
          <section className="space-y-6">
            <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-accent">01. Service Catalog</h2>
            <div className="grid gap-3">{services.map(s => (
              <button key={s.id} onClick={() => setSelService(s)} className={`p-8 rounded-card border-2 text-left transition-all flex justify-between items-center cursor-pointer ${selService?.id === s.id ? 'bg-primary text-white border-primary shadow-premium scale-[1.02]' : 'bg-white border-stone-50'}`}>
                ...
                <p className="font-black text-2xl tracking-tighter">฿{Number(s.price).toLocaleString()}</p>
              </button>
            ))}</div>
          </section>
          <section className="space-y-6">
            <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-accent">02. Select Professional</h2>
            <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">{staffs.map(st => (
              <button key={st.id} onClick={() => { setSelStaff(st); setSelTime(''); }} className={`shrink-0 flex flex-col items-center gap-4 px-10 py-10 rounded-card border-2 transition-all cursor-pointer ${selStaff?.id === st.id ? 'bg-secondary border-accent shadow-sm' : 'bg-white border-stone-50'}`}>
        ...
        <div className="lg:col-span-5">
          <section className="space-y-6 sticky top-32">
            <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-accent">03. Time Schedule</h2>
