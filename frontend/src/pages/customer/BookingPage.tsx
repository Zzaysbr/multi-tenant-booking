...
export default function BookingPage() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        // ✅ ใช้ axios.ts ตัวเก่งของเรา จัดการพ่วง Path ให้ครับ
        const [i, c] = await Promise.all([
          api.get(`/bookings/init`), 
          api.get(`/config`)
        ]);
        setServices(i.data.services || []); 
        setStaffs(i.data.staffs || []); 
        setBusinessHours(i.data.businessHours || []); 
        setShopName(c.data.config.name);
      } catch (err) {
  ...
  useEffect(() => {
    if (selStaff && selDate) {
      api.get(`/bookings/busy-slots`, { 
        params: { staffId: selStaff.id, date: selDate } 
  ...
  const confirm = async () => {
    if (!selService || !selStaff || !selTime) return toast.error("กรุณาเลือกข้อมูลให้ครบ");
    setSubmitting(true);
    try {
      ...
      toast.error(err.response?.data?.error || "จองไม่สำเร็จ");
    } finally { setSubmitting(false); }
  };

  if (loading) return <div className="h-screen flex items-center justify-center font-black animate-pulse uppercase text-accent">Preparing Booking System...</div>;
