...
export default function QueueBoard() {
  ...
  const fetchQueue = async () => {
    try {
      // ✅axios.ts จะจัดการพ่วงชื่อร้านค้าให้อยู่แล้วครับ
      const res = await api.get(`/bookings/queue`);
      setQueues({
      ...
      <header className="bg-primary text-white py-12 px-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6 relative z-10">
          <div className="text-center md:text-left space-y-2">
            ...
            <h1 className="text-5xl font-black tracking-tighter uppercase leading-none">{tenantPath?.toUpperCase()} Live</h1>
          </div>
              ...
              {queues.serving.length > 0 ? queues.serving.map((q: any) => (
                <div key={q.id} className="flex items-center justify-between p-8 bg-emerald-50/50 rounded-card border border-emerald-100 group">
                   <div className="space-y-1">
                      ...
                      <p className="text-sm font-black text-primary uppercase leading-none">{q.customerName || q.guestName}</p>
                   </div>
          ...
          <div className="card-cozy p-10! border-stone-100 h-full bg-white">
            ...
            <div className="space-y-4">
              {queues.waiting.length > 0 ? queues.waiting.map((q: any) => (
                <div key={q.id} className="flex items-center justify-between p-6 bg-stone-50 rounded-[28px] border border-stone-100 transition-all duration-500">
                ...
                <div className="py-20 text-center text-stone-200 uppercase font-black text-[10px] tracking-widest">Waiting list is empty</div>
              )}
            </div>
            <div className="mt-10 pt-8 border-t border-stone-100 flex items-center justify-between">
               <div className="flex items-center gap-3"><Users className="text-accent" size={20} /><span className="text-xs font-black text-primary uppercase tracking-widest">Total Waiting</span></div>
