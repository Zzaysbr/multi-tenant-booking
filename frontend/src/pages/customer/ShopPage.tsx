...
export default function ShopPage() {
  ...
  useEffect(() => {
    const fetchShopData = async () => {
      try {
        // ✅ ไม่ต้องใส่ /api/${tenantPath} เพราะ axios จัดการให้
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-black text-accent animate-pulse uppercase tracking-widest">Entering {tenantPath}...</div>;
  ...
  return (
    <div className="min-h-screen bg-bg font-sans text-secondary-foreground pb-32 No Italic">
      ...
      <section className="pt-28 max-w-6xl mx-auto px-6 space-y-16">
        <div className="relative h-[60vh] rounded-card overflow-hidden shadow-premium group border border-stone-100">
        ...
        <div className="space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {services.map((s: any) => (
              <div key={s.id} onClick={() => navigate(`/${tenantPath}/book`)} className="card-cozy group cursor-pointer flex flex-col justify-between p-10! bg-white border-stone-50 hover:border-accent/30 transition-all">
                <div className="flex justify-between items-start mb-10"><div className="w-16 h-16 bg-secondary rounded-[28px] flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-all duration-500"><Package size={28} /></div><div className="text-right"><p className="text-3xl font-black text-primary tracking-tighter">฿{Number(s.price).toLocaleString()}</p></div></div>
