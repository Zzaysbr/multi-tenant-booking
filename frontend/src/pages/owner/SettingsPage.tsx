...
export default function SettingsPage() {
  ...
  // ✅ ระบบจัดการ URL รูปภาพให้รองรับ Production
  ...
  if (loading) return (
    <div className="h-[60vh] flex flex-col items-center justify-center animate-pulse">
       ...
       <p className="font-black text-[10px] uppercase tracking-widest text-accent">Accessing Control Center...</p>
    </div>
  );
    ...
    <div className="max-w-6xl mx-auto space-y-12 animate-in fade-in duration-700 pb-24 font-sans">
      ...
      <form onSubmit={hSave} className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-4 space-y-10">
          ...
          {/* --- 🤖 LINE Engine (พาร์ทที่พี่ตามหา กลับมาแล้วครับ!) --- */}
          <div className="card-cozy p-10! border-stone-100 bg-stone-50/50 space-y-8 relative overflow-hidden">
            ...
            <div className="space-y-5">
              <div className="space-y-1.5">
                ...
                <input className="input-warm w-full text-xs" value={config.line_bot_id} onChange={e => setConfig({...config, line_bot_id: e.target.value})} placeholder="@yourbot" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[9px] font-black uppercase text-emerald-800/60 px-2">Owner User ID</label>
                <input className="input-warm w-full text-xs font-mono" value={config.line_user_id} onChange={e => setConfig({...config, line_user_id: e.target.value})} placeholder="U123456..." />
              </div>
              <div className="space-y-1.5">
                <label className="text-[9px] font-black uppercase text-emerald-800/60 px-2">Channel Token</label>
                <input type="password" className="input-warm w-full text-xs font-mono" value={config.line_channel_token} onChange={e => setConfig({...config, line_channel_token: e.target.value})} placeholder="••••••••" />
              </div>
            </div>
          </div>
