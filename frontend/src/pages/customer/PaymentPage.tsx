...
export default function PaymentPage() {
  ...
  // ✅ State สำหรับสลับประเภทการชำระเงิน (PromptPay หรือ บัตรเครดิต)
  const [payMethod, setPayMethod] = useState<'promptpay' | 'card'>('promptpay');
  
  // State สำหรับ PromptPay
  const [slipFile, setSlipFile] = useState<File | null>(null);
  const [slipPreview, setSlipPreview] = useState<string | null>(null);

  // ✅ State สำหรับ Mock Card
  ...
  // Submit PromptPay (อัปโหลดสลิปแบบเดิม)
  ...
  // ✅ Submit Mock Card (ยิง API ตัดบัตรจำลองที่เราเพิ่งสร้าง)
  const handleCardSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.post(`/api/${tenantPath}/bookings/${bookingId}/payment/card`);
      toast.success("ชำระเงินผ่านบัตรสำเร็จ! คิวได้รับการยืนยันแล้ว", { duration: 4000 }); 
      navigate(`/${tenantPath}/my-bookings`);
    } catch { 
      toast.error("การตัดบัตรล้มเหลว"); 
    } finally { 
      setSubmitting(false); 
    }
  };
  ...
  return (
    <div className="min-h-screen bg-bg font-sans pb-32 No Italic">
      ...
      <header className="max-w-2xl mx-auto pt-32 px-6 flex items-center justify-between">
         <button onClick={() => navigate(-1)} className="p-4 bg-white rounded-2xl border border-stone-100 hover:bg-stone-50 cursor-pointer transition-all"><ArrowLeft size={18}/></button>
      ...
      <div className="max-w-2xl mx-auto px-6 mt-12 space-y-10">
        ...
        <section className="card-cozy p-12! bg-primary text-white border-none shadow-premium">
           ...
           <div className="flex justify-between items-start mb-10">
             <div className="space-y-2">
               <h2 className="text-3xl font-black uppercase leading-none tracking-tight">{booking?.serviceName}</h2>
               <p className="text-[11px] font-black opacity-40 uppercase tracking-widest">{booking?.staffName || 'Provider Assigned'}</p>
             </div>
             <p className="text-4xl font-black text-accent tracking-tighter">฿{booking?.price}</p>
           </div>
           <div className="pt-8 border-t border-white/10 flex justify-between text-[11px] font-black uppercase tracking-widest opacity-80">
        ...
        <div className="flex bg-stone-100 p-2 rounded-2xl">
          <button onClick={() => setPayMethod('promptpay')} className={`flex-1 py-4 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all cursor-pointer ${payMethod === 'promptpay' ? 'bg-white text-primary shadow-sm' : 'text-stone-400'}`}>PromptPay</button>
          <button onClick={() => setPayMethod('card')} className={`flex-1 py-4 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all cursor-pointer ${payMethod === 'card' ? 'bg-white text-primary shadow-sm' : 'text-stone-400'}`}>Credit Card</button>
        </div>
               ...
               <div className="space-y-4">
                 ...
                 <div className="relative group">
                   ...
                   <label htmlFor="slip" className={`w-full aspect-video rounded-[48px] border-2 border-dashed flex flex-col items-center justify-center cursor-pointer overflow-hidden transition-all ${slipPreview ? 'border-accent shadow-premium' : 'border-stone-200 bg-white hover:border-accent'}`}>
                     {slipPreview ? <img src={slipPreview} className="w-full h-full object-cover animate-in fade-in" /> : <div className="text-center space-y-3"><div className="w-16 h-16 bg-secondary rounded-3xl flex items-center justify-center mx-auto text-accent"><Upload size={24}/></div><p className="text-[10px] font-black text-stone-300 uppercase tracking-widest">Tap to choose slip</p></div>}
                   </label>
                 </div>
               </div>
               <button disabled={submitting || !slipFile} className="btn-boutique-primary w-full py-6 text-base shadow-premium cursor-pointer">{submitting ? <Loader2 className="animate-spin mx-auto"/> : "Confirm Transfer"}</button>
            </form>
          </div>
        )}

        {/* --- 💳 ส่วนของ Mock Credit Card --- */}
        {payMethod === 'card' && (
          <form onSubmit={handleCardSubmit} className="animate-in fade-in zoom-in-95 duration-300 space-y-8">
            <div className="card-cozy p-10! border-stone-100 bg-white space-y-8">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-muted px-2">Card Number</label>
                  <input required type="text" maxLength={16} placeholder="4242 4242 4242 4242" className="input-warm py-4 font-mono text-sm tracking-widest" value={cardData.number} onChange={e => setCardData({...cardData, number: e.target.value})} />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-muted px-2">Cardholder Name</label>
                  <input required type="text" placeholder="JOHN DOE" className="input-warm py-4 uppercase tracking-widest" value={cardData.name} onChange={e => setCardData({...cardData, name: e.target.value})} />
                </div>
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-muted px-2">Expiry</label>
                    <input required type="text" maxLength={5} placeholder="MM/YY" className="input-warm py-4 font-mono text-center" value={cardData.exp} onChange={e => setCardData({...cardData, exp: e.target.value})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-muted px-2">CVC</label>
                    <input required type="text" maxLength={3} placeholder="123" className="input-warm py-4 font-mono text-center" value={cardData.cvc} onChange={e => setCardData({...cardData, cvc: e.target.value})} />
                  </div>
                </div>
              </div>
            </div>
            <button disabled={submitting} type="submit" className="btn-boutique-primary w-full py-6 text-base shadow-premium cursor-pointer bg-slate-800 hover:bg-slate-900 border-none">
